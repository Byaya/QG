...
public:
	...
	// 创建立方体网格数据
	template<class VertexType = VertexPosNormalTex, class IndexType = WORD>
	static MeshData<VertexType, IndexType> CreateBox(int flag,float width = 2.0f, float height = 2.0f, float depth = 2.0f,
...
template<class VertexType, class IndexType>
inline Geometry::MeshData<VertexType, IndexType> Geometry::CreateBox(int flag, float width, float height, float depth, const DirectX::XMFLOAT4 & color)
{
	using namespace DirectX;
